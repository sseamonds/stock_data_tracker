...
def check_current_cef_discount(stock:str, curr_prem_disc:float, 
    ...
    '''
    stock_info = rf.get_current_metrics_by_stock(stock, user_name, password, rds_host, db_name)
    ...
    if curr_prem_disc:
        if stock_info:
            if curr_prem_disc < 0 and stock_info['nav_discount_avg_1y'] > curr_prem_disc:
                messages.append(f'Discount for {stock} of {curr_prem_disc} has exceeded its 1 year average of {stock_info['nav_discount_avg_1y']}')
            if curr_prem_disc < 0 and stock_info['nav_discount_avg_alltime'] > curr_prem_disc:
                messages.append(f'Discount for {stock} of {curr_prem_disc} has exceeded its 1 year average of {stock_info['nav_discount_avg_alltime']}')
        else:
            raise Exception(f'Error getting stock info for {stock}')
    else:
        raise Exception(f'Error getting premium/discount for {stock}')
